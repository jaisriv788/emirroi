...
function Table({ debounceValue }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const response = await axios.post(`${baseurl}/api/withdraw_history`, {
        user_id,
      });
    ...
    <div className=" flex flex-col gap-5">
            ...
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((item, index) => (
                <tr key={index}>
                  ...
                  <td className="font-semibold">{item.comment}</td>
                  {/* <td className="font-semibold">{item.resDate}</td> */}
                </tr>
              ))
            )}
          </tbody>
      ...
      <div className="flex justify-between">
        <button
          ...
          className="bg-[#64A942] text-white w-[60px] rounded-md hover:bg-gray-700 cursor-pointer transition ease-in-out duration-300 "
        >
          Prev
        </button>
        <button
